  ...
  .controller('ChangeSettingCtrl',['$scope','$http','$mdDialog','$location','$timeout',ChangeSettingCtrl])
   ...
   function ChangeSettingCtrl($scope,$http,$mdDialog,$location,$timeout) {
    ...
    for (var i = 0; i < authoritySet.length; i++) {
        ...
        //定义需要修改的值
        $scope.config={"isRansfer":"","isNeedSecret":"","isNeedPwd":"","aboutUs":"","copyRight":"","companyPhone":"","companyAddress":"","moacAddress":"","moacSecret":"","swtcAddress":"","swtcSecret":"",};
        //弹出对话框
        ...
        //表单回显
        $http.post('http://localhost:8080/applicationMarket-server/' + 'config/getConfigInfo.do',{},{params:{
        ...
        $scope.modifySetting=function(){
            $http.post('http://localhost:8080/applicationMarket-server/' + 'config/modifyInfo.do',{},{params:{
                configId:1,
                isTransfer:$scope.config.isRansfer,
                isNeedSecret:$scope.config.isNeedSecret,
                isNeedPwd:$scope.config.isNeedPwd,
                aboutUs:$scope.config.aboutUs,
                copyRight:$scope.config.copyRight,
                companyPhone:$scope.config.companyPhone,
                companyAddress:$scope.config.companyAddress,
                moacAddress:$scope.config.moacAddress,
                moacSecret:$scope.config.moacSecret,
                swtcAddress:$scope.config.swtcAddress,
                swtcSecret:$scope.config.swtcSecret
            }}).success(function (data) {
               if (data.code == 0) {
    ...
    $scope.login = function(){
        ...
        function getSettingList(pageNum, pageSize){
            ...
            if($scope.status==""){
             ...
             $http.post('http://localhost:8080/applicationMarket-server/' + 'setting/getSettingList.do',{},{params:{
      ...
      function exportCsv(obj){
        ...
        $scope.resetPassWord=function(id){
            $http.post("http://localhost:8080/applicationMarket-server/"+"setting/resetPassword.do?",{},{params:{
                            settingId:id
                        }}).success(function (data){
                            if(data.successCode == 100200){
            ...
            $scope.startSetting = function(id){
                $scope.showConfirm = function() {
                    ...
                    .title('是否确定启用该条管理员')
                        ...
                        // console.log('确定')
                        $http.post("http://localhost:8080/applicationMarket-server/"+"setting/startSetting.do?",{},{params:{
                            settingId:id
                        }}).success(function (data){
                            if(data.successCode == 100200){
            ...
            $scope.deleteSetting = function(id){
                $scope.showConfirm = function() {
                    ...
                    .title('是否确定注销该条管理员')
                                ...
                                $mdDialog.show(confirm).then(function() {
                        // console.log('确定')
                        $http.post("http://localhost:8080/applicationMarket-server/"+"setting/logout.do?",{},{params:{
                            settingId:id
                        }}).success(function (data){
                        	if(data.successCode == 100200){
        ...
        // 获取权限列表
        $http.post('http://localhost:8080/applicationMarket-server/' + 'authority/getAuthorityList.do',{},{params:{
            pageSize:100,
            pageNum:1
        }}).success(function (data) {
            if (data.successCode == 100200) {
