(function(){
  ...
  function UserCtrl($scope,$http,$mdDialog,$location,$timeout){
    ...
    function getUserList(pageNum, pageSize){
        $http.post('http://localhost:8080/applicationMarket-server/' + 'user/getUserListBack.do',{},{params:{
            userId:$scope.userId,
    ...
    function exportCsv(obj){
                        ...
                        for(var i=0;i<data.length;i++){
                            if ($scope.selected[data[i].userId]==false) {
                                        continue;
                                    }
                            var temp = [];
                            for(var j=0;j<titleForKey.length;j++){
                                ...
                                if (titleForKey[j]=="gender") {
                                    ...
                                    // console.log("---->"+$scope.selected[data[i].userId]);
...
$scope.exportExcel = function(){
                ...
                var url="http://localhost:8080/applicationMarket-server/"+"user/exportExcel.do?";

                // var modifyTopicUrl ="http://localhost:8080/applicationMarket-server/"+"user/exportExcel.do";// 接收上传文件的后台地址
...
$scope.deleteList = function(){
                ...
                var modifyTopicUrl ="http://localhost:8080/applicationMarket-server/"+"user/deleteUserBatch.do";// 接收上传文件的后台地址
                console.log($scope.selected);
                var temp = "";
        ...
        $scope.deleteUser = function(id){
            $scope.showConfirm = function() {
                ...
                .title('是否确定删除该条用户信息')
                            ...
                            $mdDialog.show(confirm).then(function() {
                    // console.log('确定')
                    $http.post("http://localhost:8080/applicationMarket-server/"+"user/deleteById.do?",{},{params:{
                        userId:id
                    }}).success(function (data){
                        if(data.code == 0){
...
/*        $scope.cancelClick = function(){
        ...
        $http.post('http://localhost:8080/applicationMarket-server/' + 'user/getUser.do',{},{params:{
            ...
            if(data.code == 0){
                ...
                if ($scope.user.isDeveloper==0) {
                    $scope.user.isDeveloper="否";
                }else{
                    $scope.user.isDeveloper="是";
                }
        ...
        $scope.unbindingDecedent=function(decedentId){
                    ...
                    .title('是否确定解除绑定信息')
                                ...
                                .cancel('取消');
                                $mdDialog.show(confirm).then(function() {
            $http.post('http://localhost:8080/applicationMarket-server/' + 'user/unbindingDecedent.do',{},{params:{
            userId:$scope.userId,  //用户id
        ...
        $scope.resetPassword=function(userId){
                    ...
                    .title('是否确定重置密码')
                                ...
                                .cancel('取消');
                                $mdDialog.show(confirm).then(function() {
            $http.post('http://localhost:8080/applicationMarket-server/' + 'user/restPassword.do',{},{params:{
            userId:$scope.userId  //用户id
        }}).success( function (data){   
            if(data.code == 0){  //解除成功
                $scope.showAlert(data.successMessage);
    ...
    function DecedentCtrl($scope,$http,$location,$mdDialog,$timeout,$sce){
        ...
        console.log("id="+$scope.userId);
        //根据用户id获取用户详细信息
        $http.post('http://localhost:8080/applicationMarket-server/' + 'user/getUserById.do',{},{params:{
            userId:$scope.userId   //用户id
        }}).success( function (data){   
            if(data.code == 0){
                $scope.user = data.result;
        ...
        //根据用户id获取其绑定的逝者信息
        $http.post('http://localhost:8080/applicationMarket-server/' + 'user/getDetailedInformation.do',{},{params:{
            userId:$scope.userId   //用户id
        }}).success( function (data){   
            if(data.code == 0){
...
// -------------------------------------------------------------------------------------------------------------
    ...
    //获取用户列表
    function getAddress(){
        $http.post('http://localhost:8080/applicationMarket-server/' + 'wallet/getAddress.do',{},{params:{
            userId:$scope.userId,
    ...
    //获取用户列表
    function getUserList(pageNum, pageSize){
        $http.post('http://localhost:8080/applicationMarket-server/' + 'wallet/getWalletDetailBack.do',{},{params:{
            userId:$scope.userId,
            walletType:$scope.walletType,
            pageNum:pageNum,
            pageSize:pageSize
...
$scope.deleteList = function(){
       ...
       function UserAddCtrl($scope,$http,$location,$mdDialog,$timeout){
        ...
        $scope.sendCode=function(mobile){
            console.log(mobile);
            $http.post("http://localhost:8080/applicationMarket-server/"+"user/genAuthCode.do?",{},{params:{
                        mobile:mobile
                    }}).success(function (data){
                        if(data.code == 0){
        ...
        $scope.adduser = function(){
            ...
            $scope.showConfirm = function() {
                    ...
                    $http.post("http://localhost:8080/applicationMarket-server/"+"user/addUser.do?",{},{params:{
