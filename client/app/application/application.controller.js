(function () {
    ...
    function ApplicationDetailCtrl($scope, $http, $mdDialog, $location, $timeout) {
        ...
        //如果返回列表
        $scope.backClick = function () {
            $location.path("/application/application-list").search({
        ...
        $scope.Plateform=$location.search().Plateform;
        $scope.Account=$location.search().Account;
        $scope.ExaminStatus=$location.search().ExaminStatus;
        $scope.GroudStatus=$location.search().GroudStatus;
        $scope.IsRecommend=$location.search().IsRecommend;
        $scope.isMust=$location.search().isMust;
        // $scope.GroudStatus=$location.search().GroudStatus;
    ...
    function ApplicationCtrl($scope, $http, $mdDialog, $location, $timeout) {
        ...
        $scope.pageNum=$location.search().pageNum;
        $scope.pageSize=$location.search().pageSize;
        $scope.ID=$location.search().AppId;
        $scope.AppName=$location.search().AppName;
        $scope.Plateform=$location.search().Plateform;
        $scope.Account=$location.search().Account;
        $scope.ExaminStatus=$location.search().ExaminStatus;
        ...
        if ( $scope.ExaminStatus==undefined) {
            $scope.ExaminStatus="";
        }

        if ( $scope.GroudStatus==undefined) {
            $scope.GroudStatus="";
        }

        if ( $scope.IsRecommend==undefined) {
            $scope.IsRecommend="";
        }

        if ( $scope.isMust==undefined) {
            $scope.isMust="";
        }
        ...
        $scope.count=1;
        $scope.select1=function select(page) {
            // if () {}
        ...
        $scope.onNumPerPageChange=function onNumPerPageChange() {
            $scope.select1($scope.currentPage);
